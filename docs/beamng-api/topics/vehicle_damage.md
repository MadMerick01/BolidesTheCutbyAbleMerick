...
- Use vehicle-context APIs for wheels, beams, and breakgroups (guarded, not listed in the GE dump).【F:lua/ge/extensions/events/BulletHit.lua†L324-L351】【F:lua/ge/extensions/events/deflateRandomTyre.lua†L54-L77】
...
## Notes / gotchas
- `veh:queueLuaCommand(...)` and `be:queueObjectLuaCommand(...)` are widely used but not listed in the GE dump; always wrap with guards and fallbacks when calling them from GE code.【F:lua/ge/extensions/events/BulletHit.lua†L343-L351】
...
- Bullet hit logic builds a payload and queues it inside the player vehicle to deform parts, break beams, and deflate tires with a safe fallback path.【F:lua/ge/extensions/events/BulletHit.lua†L324-L369】
