...
- Prefer `veh:queueLuaCommand(...)` on a vehicle object when available for direct control (guarded, not in dump).【F:lua/ge/extensions/events/BulletHit.lua†L343-L351】
...
- Vehicle Lua context is a separate VM: avoid heavy GE calls and pass only needed data in the queued payload string to keep execution deterministic.【F:lua/ge/extensions/events/BulletHit.lua†L324-L351】
