...
- `ImTextureHandler` (callable table, returns an `ImTextureID`)
...
## Notes / gotchas
- Prefer `ImTextureHandler(path)` for loading textures in ImGui. It is present in the 0.38 dump as a callable table and works with `Image` calls. `LoadTexture` is not listed in the dump, so treat it as a legacy fallback if it exists at runtime.
