-- lua/ge/extensions/events/BankVan.lua
-- BankVan: manual spawn at ForwardKnownBreadcrumb(200m)
-- Behavior:
--   1) Spawn bank van at ForwardKnownBreadcrumb(200m).
--   2) Van immediately FLEES at a safe pace (80 km/h max) and does not pursue.
--   3) When within 50m, van fires shotgun damage at fixed intervals.
--   4) End when van escapes to >800m or when player catches/stops it.
...
local function log(msg)
  ...
  if Host and Host.postLine then
    Host.postLine("BANKVAN", R.status)
  else
    print("[BankVan] " .. tostring(R.status))
  end
end
...
local function queueAI_FollowLegal(veh, targetId)
  veh:queueLuaCommand(([[
    local function try(desc, fn)
      ...
      if not ok then print("[BankVan AI] FAIL: "..desc.." :: "..tostring(err)) end
      return ok
    end

    local tid = %d
    if not ai then print("[BankVan AI] ai missing"); return end
    ...
    print("[BankVan AI] FOLLOW armed (legal, lane changes, avoid cars/crash). targetId="..tostring(tid))
  ]]):format(targetId))
end
...
local function queueAI_Flee(veh, targetId)
  veh:queueLuaCommand(([[
    ...
    print("[BankVan AI] FLEE armed (max 80kph, legal, lane changes, avoid cars/crash). targetId="..tostring(tid))
  ]]):format(targetId))
end

local startFleeAI

startFleeAI = function(robberId)
  ...
  if not targetId then
    log("BankVan AI: waiting for valid player targetId...")
    return false
  end
  ...
  R.nextShotAt = nil
  log("BankVan AI: FLEE (max 80kph, legal, lane changes, avoid cars/crash). targetId=" .. tostring(targetId))
  return true
end
...
local function beginActiveRun(id)
  ...
  setHud(
    "event",
    "The locals are reporting a Bolide Money Van nearby",
    "Stop the van before they move profits out of town, be careful they are armed and dangerous",
    nil
  )
end
...
local function updateStartupPipeline()
  ...
  if R.startupStage == "aiStart" then
    if R.spawnedId then
      local ok = startFleeAI(R.spawnedId)
      if ok then
        R.aiStarted = true
...
function M.update(dtSim)
  ...
  if d > 800.0 then
    local msgArgs = {
      title = "NOTICE",
      text = "The Bolides have successfully moved their profits out of the area",
      freeze = true,
  ...
  if R.spawnPos and R.spawnClock and (now - R.spawnClock) <= 2.0 and not R.spawnSnapped then
    ...
    if spawnDist >= 50.0 then
      ...
      R.spawnSnapped = true
      if ok then
        log("BankVan AI: snapped back to spawn (anti-teleport).")
        startFleeAI(R.spawnedId)
      else
  ...
  if d <= 50.0 then
    if not R.shotsStarted then
      ...
      setHud(
        "danger",
        "The Bolide money van is firing at you",
        "Stop the van before they move profits out of town, be careful they are armed and dangerous",
        "shotsFired"
      )
    end
